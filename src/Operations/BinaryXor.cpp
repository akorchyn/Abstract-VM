#include "../../Includes/Operations/BinaryXor.hpp"

BinaryXor::BinaryXor(int line) : line(line)
{
}

bool BinaryXor::execute(TypeStack& stack) const
{
	if (stack.size() < 2)
		throw AbstractRuntimeException(std::string("BinaryXor error: Line ") + std::to_string(line) + ": Stack size less then 2");
	...
	try {
		...
		throw AbstractRuntimeException(std::string("BinaryXor error: Line ") + std::to_string(line) + ": "  + x.what());
	}
	return true;
}