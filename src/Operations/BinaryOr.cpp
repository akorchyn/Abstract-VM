#include "../../Includes/Operations/BinaryOr.hpp"

BinaryOr::BinaryOr(int line) : line(line)
{
}

bool BinaryOr::execute(TypeStack& stack) const
{
	if (stack.size() < 2)
		throw AbstractRuntimeException(std::string("BinaryOr error: Line ") + std::to_string(line) + ": Stack size less then 2");
	...
	try {
		...
		throw AbstractRuntimeException(std::string("BinaryOr error: Line ") + std::to_string(line) + ": "  + x.what());
	}
	return true;
}
