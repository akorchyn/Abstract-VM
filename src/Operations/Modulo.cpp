#include "../../Includes/Operations/Modulo.hpp"

Modulo::Modulo(int line) : line(line)
{
}

bool Modulo::execute(TypeStack& stack) const
{
	if (stack.size() < 2)
	...
	try {
		...
		throw AbstractRuntimeException(std::string("Modulo error: Line ") + std::to_string(line) + ": "  + x.what());
	}
	return true;
}
